    ...
    subsections: [
      {
        ...
        content: "The frontier of production AI isn't just training better models—it's making them reliable, composable, and safe at scale. I've spent the last year building infrastructure that does exactly that.\n\n<strong>At AdsgencyAI (Jun 2026):</strong>\nDelivered a flagship competitor-ad discovery platform for 500+ users by engineering the full stack: frontend masonry UI, backend Meta/TikTok REST APIs, and an LLM-based classification engine that intelligently routes, normalizes, and caches cross-platform ad creatives. Eliminated critical production pain: worst-case wait times were 15–20 minutes for paid-tier users because ad-generation jobs were hitting rate limits and cascading failures. I engineered a platform-wide Celery priority-queue system, implementing reliable job processing at scale. The system now handles ~200 AI-generation jobs daily with millisecond-level latency.\n\nEngineered production LLM reliability infrastructure from scratch: Redis-backed concurrency semaphores to limit concurrent API calls, jittered exponential backoff with Retry-After support, and a circuit breaker across the model gateway. This prevented provider rate limits from cascading into platform-wide failures—a common failure mode in systems that orchestrate multiple LLM calls.\n\nHardened the image-ingestion pipeline against infrastructure vulnerabilities, implementing defense-in-depth with a thread-safe secure connection pool and strict certificate verification to prevent attackers from intercepting or spoofing image requests.\n\nRedesigned an unbounded LLM API call into an asynchronous, semaphore-capped worker pool, eliminating a 5–15 minute first-load hang for AI recommendations and delivering instant fallback responses to users.\n\nEngineered the backend foundation for visual discovery by integrating TikTok and Meta Ads APIs: built a unified data normalizer, caching layer, and pagination system. Enhanced search reliability by implementing lazily-triggered pagination with cross-page cache deduplication, ensuring users receive at least 15 unique, high-quality creatives per query.\n\nFixed a Redis cache bypass affecting 31 LangGraph agent nodes and consolidated duplicated 'no-tools' LLM fallback logic copy-pasted across 27 files onto a single shared handler, reducing code duplication and improving reliability.\n\nFixed critical frontend and authentication bugs including Roku OAuth stale connection handling and silent token-refresh failures, improving platform stability.\n\n<strong>At NYU IT (May 2025 – May 2026):</strong>\nProposed and independently built custom Model Context Protocol (MCP) servers for GitHub and Google Workspace—not an assigned project. These servers enable AI agents to autonomously triage and resolve internal repository issues, saving ~40 engineering hours weekly.\n\nDeployed a Snowflake MCP integration allowing non-technical university stakeholders to run natural-language analytical queries against institutional data, reducing analyst bottlenecks. Extended the workflow with a RAG layer using pgvector and the Claude API; built a streaming LLM response interface in React using Server-Sent Events (SSE).",
        projects: [
      ...
      },
      {
        ...
        content: "Building data systems requires different thinking: you're optimizing for accuracy, freshness, and operational stability simultaneously. I've built pipelines that ingest real-time data, build features, train models, and serve predictions—all while remaining observable and debuggable.\n\n<strong>MTA NYC Bus Transit Time Prediction:</strong>\nBuilt a real-time predictive pipeline ingesting REST APIs into a PostgreSQL feature store, training spatially partitioned XGBoost models per NYC grid zone. The work involves data pipeline design (ingestion, transformation, storage), feature engineering (spatial partitioning, temporal features), model training, and serving. Real-time prediction pipelines add constraints: you need low-latency predictions, reliable ingestion, and a feature store that can handle high-throughput reads.\n\n<strong>RAG-Powered Institutional Knowledge Assistant:</strong>\nBuilt a RAG system over NYU IT documentation using pgvector and Flask, combining dense vector similarity (embeddings via Claude API) with keyword BM25 filtering for high-relevance answers. RAG systems are a practical way to ground LLM responses in domain-specific data without fine-tuning; they require thoughtful design of the retrieval pipeline and careful prompt engineering to balance relevance and generation quality.",
        projects: [
    ...
    subsections: [
      {
        ...
        snippet: "Making models reliable, composable, and safe at scale is the frontier.",
        content: "The frontier isn't just training better models—it's making them reliable, composable, and safe at scale. Right now (July 2026), we're in the phase where LLMs are powerful but brittle. They hallucinate, they're rate-limited, they fail silently, and they don't integrate cleanly into existing systems.\n\nBuilding production LLM infrastructure that solves these problems is where I want to focus next. This means:\n\n<strong>Reliability:</strong> Circuit breakers, rate limiting, retry strategies, and graceful degradation when models are unavailable or expensive. The patterns are borrowed from distributed systems (which have been solving these problems for 20 years), but they're still not standard in the LLM space.\n\n<strong>Composability:</strong> Model Context Protocol and similar standards are enabling AI agents to integrate with external systems (APIs, databases, code repos, Slack). The next phase is orchestrating multi-step workflows reliably—having an agent decompose a problem, route different steps to different models or APIs, and handle failures gracefully.\n\n<strong>Safety & Observability:</strong> As LLMs handle more critical tasks, we need better ways to audit their decisions, detect hallucinations, and ensure they're operating within bounds. This involves logging, tracing, and novel validation strategies.\n\nI've built pieces of this infrastructure already (at AdsgencyAI and NYU). I want to go deeper—designing systems that become the standard for how teams integrate LLMs at scale.",
      },
      {
        title: "Breadth, Not Narrow Specialization",
        snippet: "Distributed systems + LLM infrastructure + intentional breadth.",
        content: "My sweet spot is at the intersection of multiple disciplines: distributed systems thinking, event-driven architecture, cloud infrastructure, and now agentic AI. Right now, that means LLM infrastructure and agentic workflows are my primary focus. But I've deliberately maintained depth in event-driven payments systems, platform development, and systems optimization to avoid tunnel vision.\n\nHard problems span domains. Most engineers specialize deeply in one domain (payments, mobile, frontend) and miss patterns that cross boundaries. I've intentionally stayed broad—not shallow, but genuinely knowledgeable across multiple areas—because the best architectural decisions require thinking across systems.\n\nThat said, here's where I have genuine depth:\n- <strong>Event-driven architecture and microservices</strong> (Amazon: 3 years, $50M/month in transactions)\n- <strong>Distributed systems</strong> (coursework, projects, production experience)\n- <strong>AWS infrastructure</strong> (Lambda, ECS, DynamoDB, Athena, Kafka, etc.; production at scale)\n- <strong>LLM reliability and agentic workflows</strong> (applying systems rigor to AI infrastructure; AdsgencyAI, NYU; focused recent work)\n- <strong>Full-stack platform development</strong> (NYU IT: Flask + React, end-to-end)\n- <strong>Systems optimization</strong> (GPU work, concurrency, profiling)\n\nThe thread running through all of it is comfort with complexity and the ability to ship under ambiguity. I can move between domains because I think in principles, not just patterns.",
        skillsPreview: true,
      },
      {
        title: "Learning & Growth",
        snippet: "On-going development areas and what's next technically.",
        content: "I'm deliberately investing in three areas over the next 12–18 months:\n\n<strong>1. Multi-agent orchestration at scale</strong>\nGoing beyond single-agent systems (e.g., a coder agent that generates PRs) to systems where multiple agents collaborate, decompose problems, and handle failures gracefully. This combines agentic AI with distributed systems thinking—two areas where I already have depth.\n\n<strong>2. LLM observability and validation</strong>\nAs LLMs handle higher-stakes tasks, we need better tooling to audit their decisions and detect when they've wandered outside bounds. This is an emerging field where patterns are still being established.\n\n<strong>3. Rust + systems languages in production</strong>\nI have solid C++ experience from coursework and projects. I want to develop production Rust expertise—for systems work where safety and concurrency matter (e.g., building infrastructure components that need to be bulletproof).\n\nThe learning approach will be the same as always: build, iterate, ship to production, learn from operational reality.",
      },
      {
        title: "Get in Touch",
        snippet: "Always happy to talk shop or hear interesting problems.",
        content: "I'm interested in conversations about:\n- Building production LLM infrastructure\n- Distributed systems, payments, and scale\n- Incident response and architectural leadership\n- Emerging technologies and how to integrate them reliably\n\nFeel free to reach out. Whether it's a full-time role, a consulting project, or just a good technical conversation, I'm listening.\n\n<strong>Contact:</strong>\n- Email: tejaschoudhary256@gmail.com\n- Phone: (551) 362-7539\n- LinkedIn: https://www.linkedin.com/in/tejaskc\n- GitHub: https://www.github.com/tejasc256",
        contact: true,
      },
      {
        title: "Education & Credentials",
        snippet: "M.S. from NYU Courant Institute (3.9 GPA), B.Tech. from NIT Karnataka.",
        content: "<strong>M.S. Computer Science</strong> — New York University, Courant Institute of Mathematical Sciences (Aug 2024 – May 2026, GPA: 3.9)\n\nI chose Courant specifically for its systems-level focus and rigorous distributed computing curriculum. The coursework—particularly Distributed Systems, Operating Systems, and Advanced Database Systems—reinforced the architectural thinking that underpins my production work across payments infrastructure and platform systems.\n\nEqually important: courses in Machine Learning and GPUs provided foundational depth for the LLM infrastructure and systems optimization work I've pursued at AdsgencyAI and NYU IT. This combination of systems rigor (distributed computing, OS fundamentals, advanced databases) plus machine learning is precisely what makes LLM reliability infrastructure work rigorous rather than hype-driven. Building production-grade circuit breakers, observability, and failure-handling for LLMs isn't a matter of ML expertise alone—it's understanding how distributed systems principles apply to AI orchestration.\n\nCourant is known for rigorous theoretical foundations; the program strengthened my ability to reason about systems at scale and design solutions that work under constraints.\n\n<strong>Relevant coursework:</strong> Distributed Systems, Machine Learning, GPUs, Advanced Database Systems, Big Data Application Development, Operating Systems, Data Communication and Networking, Fundamental Algorithms\n\n<strong>B.Tech. Computer Science &amp; Engineering</strong> — National Institute of Technology Karnataka (Jul 2017 – May 2021)\n\nNIT Karnataka is one of India's premier engineering colleges. The program provided a strong foundation in algorithms, data structures, systems thinking, and computer architecture that I've built upon throughout my career. This foundation—rigorous and theory-grounded—shaped my approach to treating engineering problems as design problems that reward deep thinking.",
      },
    ],
  },
};
